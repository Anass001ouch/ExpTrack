...
interface ProductCatalogViewProps {
  ...
  onAddProduct: (product: { name: string; category: string; sku: string; image_url: string }) => Promise<void>;
...
export default function ProductCatalogView({
  ...
  const getBatchCount = (productId: string) =>
    batches.filter((b) => b.product_id === productId).length;

  const getNearestExpiry = (productId: string) => {
    ...
    const expiry = new Date(nearest.expiration_date);
    expiry.setHours(0, 0, 0, 0);
    const days = Math.ceil((expiry.getTime() - now.getTime()) / 86400000);
    return { batch: nearest, days };
  };
        ...
        <div className="mx-auto max-w-4xl px-6 py-8 lg:px-10">
          ...
          >
            <div>
              <button
                onClick={onBack}
                className="mb-3 flex items-center gap-1.5 text-xs font-medium text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] transition-colors"
              >
          ...
          <div className="space-y-2">
            <AnimatePresence>
              {products.map((product, index) => {
                const batchCount = getBatchCount(product.id);
                const nearest = getNearestExpiry(product.id);
                const urgency = nearest ? getUrgencyLevel(nearest.days) : 'safe';

                const urgencyIcon =
                  urgency === 'critical' ? (
                    <AlertTriangle className="h-3.5 w-3.5 text-critical-accent" />
                  ) : urgency === 'warning' ? (
                    <Clock className="h-3.5 w-3.5 text-warning-accent" />
                  ) : (
                    <ShieldCheck className="h-3.5 w-3.5 text-safe-accent" />
                  );
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => onNavigateToBatches(product.id)}
                    className="group cursor-pointer rounded-xl border border-[var(--border-secondary)] bg-[var(--bg-card)] p-4 transition-all duration-200 hover:shadow-[var(--shadow-md)] hover:border-[var(--border-primary)]"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      {product.image_url ? (
                        ...
                        <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-lg bg-[var(--bg-secondary)]">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <ChevronRight className="h-4 w-4 text-[var(--text-tertiary)] transition-transform duration-200 group-hover:translate-x-0.5" />
                      </div>
                    </div>
                  </motion.div>
                );
...
interface AddProductModalProps {
  onAdd: (product: { name: string; category: string; sku: string; image_url: string }) => Promise<void>;
  onClose: () => void;
  isLoading: boolean;
  setIsLoading: (v: boolean) => void;
}

function AddProductModal({ onAdd, onClose, isLoading, setIsLoading }: AddProductModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await onAdd({ name, category, sku, image_url: imageUrl });
      onClose();
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-md rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-elevated)] p-6 shadow-[var(--shadow-xl)]"
      >
        ...
        <div className="mb-5">
          {imageUrl ? (
            <div className="relative h-32 w-full overflow-hidden rounded-xl">
              ...
              <button
                onClick={() => setImageUrl('')}
                className="absolute top-2 right-2 rounded-lg bg-black/50 p-1.5 text-white/80 hover:bg-black/70 transition-colors"
              >
            ...
            <button
              ...
              className="flex h-32 w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[var(--border-primary)] bg-[var(--bg-primary)] text-sm text-[var(--text-tertiary)] hover:border-[var(--text-tertiary)] hover:text-[var(--text-secondary)] transition-colors"
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              className="w-full rounded-xl border border-[var(--border-primary)] bg-[var(--bg-primary)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder-[var(--text-tertiary)] outline-none transition-all duration-200 focus:border-[var(--accent-primary)] focus:ring-1 focus:ring-[var(--accent-primary)]/10"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              required
              className="w-full rounded-xl border border-[var(--border-primary)] bg-[var(--bg-primary)] px-4 py-3 text-sm text-[var(--text-primary)] outline-none transition-all duration-200 focus:border-[var(--accent-primary)] focus:ring-1 focus:ring-[var(--accent-primary)]/10 appearance-none"
            >
          ...
          <div>
            ...
            <div className="relative">
              <Upload className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--text-tertiary)]" strokeWidth={1.5} />
              <input
                type="text"
                value={sku}
                onChange={(e) => setSku(e.target.value)}
                placeholder="Optional"
                className="w-full rounded-xl border border-[var(--border-primary)] bg-[var(--bg-primary)] pl-10 pr-4 py-3 text-sm text-[var(--text-primary)] placeholder-[var(--text-tertiary)] outline-none transition-all duration-200 focus:border-[var(--accent-primary)] focus:ring-1 focus:ring-[var(--accent-primary)]/10"
              />
            </div>
          </div>
          ...
          <motion.button
            ...
            className="w-full rounded-xl bg-[var(--accent-primary)] px-5 py-3 text-sm font-medium text-[var(--text-inverse)] transition-colors hover:bg-[var(--accent-hover)] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isLoading ? (
